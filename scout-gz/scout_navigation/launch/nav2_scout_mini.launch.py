...
def generate_launch_description():
    ...
    nav2_scout_dir = get_package_share_directory('scout_navigation')
    nav2_scout_launch_dir = os.path.join(nav2_scout_dir, 'launch')
    sim_dir = get_package_share_directory('scout_description')
    ...
    declare_robot_name_cmd = DeclareLaunchArgument(
        'robot_name', default_value='scout_mini', description='name of the robot'
    )

    declare_robot_sdf_cmd = DeclareLaunchArgument(
        'robot_sdf',
        default_value=os.path.join(sim_dir, 'sdf', 'model.sdf'),
        description='Full path to robot sdf file to spawn the robot in gazebo',
    )

    sdf = os.path.join(sim_dir, 'sdf', 'model.sdf')
    ...
    bringup_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(nav2_scout_launch_dir, 'bringup_launch.py')),
        launch_arguments={
            'namespace': namespace,
            'use_namespace': use_namespace,
            'slam': slam,
    ...
    # Inicia o script de ground truth
    start_ground_truth_node_cmd = Node(
            package='scout_navigation',
            executable='gt.py',
            name='ground_truth_node',
            output='screen'
        )
    ld.add_action(start_ground_truth_node_cmd)
